<!DOCTYPE html>
{{-- Halaman LOGIN - satu layar berisi form masuk + form daftar + panel biru geser --}}
...
<head>
    ...
    {{-- Muat CSS + JS khusus halaman login via Vite --}}
    @vite(['resources/css/auth/login.css', 'resources/js/auth/login.js'])
</head>
<body>

{{-- Tentukan panel yg tampil: ikut form yg terakhir di-submit, default-nya form masuk (login) --}}
@php($activeForm = old('auth_form', 'login'))
...
<main class="auth-wrap">
    ...
    <div class="container {{ $activeForm === 'register' ? 'right-panel-active' : '' }}" id="container" data-active-form="{{ $activeForm }}">
        {{-- PANEL DAFTAR (tersembunyi, muncul saat slider digeser) --}}
        <section class="form-container sign-up-container" aria-label="Formulir pendaftaran">
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                <input id="reg_name" type="text" name="name" value="{{ $activeForm === 'register' ? old('name') : '' }}" required autocomplete="name" />
        ...
        {{-- PANEL MASUK (tampil default di halaman login) --}}
        <section class="form-container sign-in-container" aria-label="Formulir masuk">
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <input id="email" type="email" name="email" value="{{ $activeForm === 'login' ? old('email') : '' }}" required autofocus autocomplete="username" />
                @if ($activeForm === 'login')
                    @foreach ((array) $errors->get('email') as $msg)
                        <p class="field-error" role="alert">{{ $msg }}</p>
                    @endforeach
